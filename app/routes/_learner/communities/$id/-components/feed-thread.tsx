      ...
      thread?.images?.length ? (
        <div className="flex w-full flex-wrap items-center gap-2">
          {thread?.attachments &&
            ...
            thread?.attachments.slice(0, 3).map((att) => {
              return (
          ...
          {thread?.attachments && thread?.attachments.length > 3 && (
            <Tag.Root>
              +{thread?.attachments.length - 3} File
              {thread?.attachments.length - 3 > 1 ? "s" : ""}
            </Tag.Root>
          )}
        </div>
      ) : null}

      <p className="mt-4 text-title-h6">{thread?.title}</p>
