...
export function WeekView({
  ...
  return (
    <div data-slot="week-view" className="flex h-full flex-col">
      <div className="border-border/70 sticky -top-2 z-30 grid grid-cols-8 items-center border-y bg-white-alpha-24 uppercase backdrop-blur-md dark:bg-black-alpha-24">
        ...
        {days.map((day) => (
          <div
            key={day.toString()}
            className={cn(
              "text-center text-text-soft-400 *:text-label-xs data-[today=true]:font-medium data-[today=true]:text-text-strong-950",
              {
                "font-medium text-text-strong-950": isToday(day),
              }
            )}
      ...
      {showAllDaySection && (
        <div className="border-border/70 border-b bg-bg-weak-50">
          <div className="grid grid-cols-8">
            <div className="border-border/70 relative border-r">
              <span className="absolute bottom-0 left-0 h-6 w-16 max-w-full pe-2 text-right text-[10px] text-text-soft-400 sm:pe-4 sm:text-label-xs">
                All day
              </span>
            </div>
      ...
      <div className="grid flex-1 grid-cols-8 overflow-hidden">
        <div className="border-border/70 grid auto-cols-fr border-r">
          {hours.map((hour, index) => (
            ...
            >
              {index > 0 && (
                <span className="absolute -top-3 left-0 flex h-6 w-16 max-w-full items-center justify-end bg-bg-white-0 pe-2 text-[10px] text-text-soft-400 sm:pe-4 sm:text-label-xs">
                  {format(hour, "h a")}
                </span>
              )}
            </div>
