...
export type ISectionProps<C extends React.ElementType = "section"> =
  VariantProps<typeof sectionVariants> &
    Omit<
      React.ComponentPropsWithoutRef<C>,
      keyof VariantProps<typeof sectionVariants>
    > & {
...
const Section = <C extends React.ElementType = "section">({
  side,
  className,
  size,
  spacer,
  ...
  return (
    <Component
      {...rest}
      className={sectionVariants({ side, size, spacer, className })}
    >
      {children}
    </Component>
  )
